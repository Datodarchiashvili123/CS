  ...
  lessons.push({
    ...
    createSimulation: function (container, setChallengeResult) {
      ...
      const layers = [
        ...
        { to: "multibit", title: "მრავალბიტიანი", desc: "ripple შემკრები + რეგისტრი (ბაიტი)" },
        ...
        { to: "two-state", title: "ორი მდგომარეობა", desc: "რატომ 0/1 — ხმაურმედეგი" },
      ...
      container.append(
        ...
        CFZ.el("p", {
          ...
          text: "ელექტრონი → ორი მდგომარეობა → ბიტი → ტრანზისტორი → კარიბჭე → შეკრება → მეხსიერება → მრავალბიტი → ALU → CPU → პროგრამა → ქეში. ეს არის კომპიუტერი ნულიდან.",
        }),
        CFZ.el("div", { className: "recap-next" }, [
          CFZ.el("h4", { text: "სად გავაგრძელო?" }),
          CFZ.el("p", {
            className: "breakdown-note",
            text: "შემდეგ სექციაში შეკრებილია ამ თავის ყველა დამატებითი რესურსი — ვიდეოები, ინტერაქტიული სიმულატორები, სრული კურსები და საცნობარო მასალა.",
          }),
          CFZ.createButton("დამატებითი რესურსები →", "primary", function () {
            CFZ.goToLesson("extra-resources");
          }),
        ])
      );
