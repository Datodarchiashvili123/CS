(function () {
  ...
  lessons.push({
    ...
    theory: "სრული შემკრები (Full Adder) სამ ბიტს კრებს — A, B და Carry In (წინა სვეტიდან გადმოსული 1). სწორედ Carry In-ის მიღების უნარი განასხვავებს მას ნახევრად შემკრებისგან და ხდის მას „სრულს“: ის მთელ სვეტს ითვლის — A + B + Cin. ამის წყალობით შემკრებების ჯაჭვად მიბმა და ნებისმიერი სიგრძის რიცხვების შეკრებაა შესაძლებელი — ეს ჯაჭვი მუშაობს ALU-ს შიგნით.",
    analogy: "ქაღალდზე სვეტებად შეკრებისას წინა სვეტიდან გადმოსული 1-ის გათვალისწინება გიწევს. Full Adder ზუსტად ამას აკეთებს — ითვლის A + B + Carry In-ს.",
    physical: "SUM = A XOR B XOR Cin, CARRY = (A AND B) OR (Cin AND (A XOR B)). შიგნით ორი XOR, ორი AND და ერთი OR კარიბჭეა — ფაქტობრივად ორი ნახევრად შემკრები + ერთი OR.",
      ...
      container.append(
        ...
        CFZ.el("div", { className: "breakdown-panel" }, [
          ...
          CFZ.el("p", {
            ...
            text: "ორი XOR ითვლის SUM-ს (A, B და Carry In), ორი AND და ერთი OR კი ქმნიან CARRY-ს. ერთ წინადადებად: Full Adder = A + B + Carry In (= ორი Half Adder + ერთი OR). აქტიური მავთულები მწვანედ ინთება.",
          }),
          diagram.element,
        ]),
        table.element,
