(function () {
  ...
  lessons.push({
    ...
    shortTitle: "პროგრამა",
    theory: "CPU პროგრამას მიჰყვება — ინსტრუქციების სიას. საათი (clock) არის „გულისცემა“: ყოველი ტიკით CPU ერთ ნაბიჯს დგამს. თითო ინსტრუქცია სამ ფაზას გადის: Fetch (აღება) → Decode (გაშიფვრა) → Execute (შესრულება).",
    analogy: "ჰგავს რეცეპტს: ნაბიჯ-ნაბიჯ კითხულობ და ასრულებ. საათი მეტრონომია, რომელიც ტემპს განსაზღვრავს.",
    physical: "საათი წამში მილიარდობით ტიკს იძლევა (გიგაჰერცები). ყოველი ტიკი სინქრონიზებს ყველა ნაწილს, რომ ბიტები სწორ დროს გადავიდნენ რეგისტრებსა და ALU-ს შორის.",
    ...
    createSimulation: function (container, setChallengeResult) {
      ...
      function highlightPC() {
        instrRows.forEach(function (row, i) {
          row.classList.toggle("is-current", i === pc && !running ? false : i === pc);
